...
/// `OldWriteSet` contains all access paths that one transaction modifies. Each of them is a `WriteOp`
...
/// A mutable version of `OldWriteSet`.
///
/// This is separate because it goes through validation before becoming an immutable `OldWriteSet`.
#[derive(Clone, Debug, Default, Eq, Hash, PartialEq, Serialize, Deserialize)]
