...
use crate::{node_type::{LeafNode, Node, NodeKey}, HashValueKey, NodeBatch, StaleNodeIndex, TreeReader, TreeUpdateBatch, TreeWriter, RawKey};
...
#[derive(Default)]
pub struct MockTreeStringStore(
    RwLock<(
        HashMap<NodeKey, Node<StringKey>>,
        BTreeSet<StaleNodeIndex>,
    )>,
);

impl TreeReader<StringKey> for MockTreeStringStore {
    fn get_node_option(&self, node_key: &NodeKey) -> Result<Option<Node<StringKey>>> {
        Ok(self.0.read().unwrap().0.get(node_key).cloned())
    }

    fn get_rightmost_leaf(&self) -> Result<Option<(NodeKey, LeafNode<StringKey>)>> {
        let locked = self.0.read().unwrap();
        let mut node_key_and_node: Option<(NodeKey, LeafNode<StringKey>)> = None;
...
impl TreeWriter<StringKey> for MockTreeStringStore {
    fn write_node_batch(&self, node_batch: &NodeBatch<StringKey>) -> Result<()> {
        let mut locked = self.0.write().unwrap();
        for (node_key, node) in node_batch.clone() {
            assert_eq!(locked.0.insert(node_key, node), None);
        }
...
impl MockTreeStringStore {
    pub fn put_node(&self, node_key: NodeKey, node: Node<StringKey>) -> Result<()> {
        match self.0.write().unwrap().0.entry(node_key) {
            Entry::Occupied(o) => bail!("Key {:?} exists.", o.key()),
            Entry::Vacant(v) => {
                v.insert(node);
    ...
    pub fn write_tree_update_batch(&self, batch: TreeUpdateBatch<StringKey>) -> Result<()> {
        batch
            .node_batch
            .into_iter()
            .map(|(k, v)| self.put_node(k, v))
